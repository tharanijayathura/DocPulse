...
router.post('/receipt/doctor', auth, async (req, res) => {
  const { patientName, ageGroup, disease, medicines, doctorCharges } = req.body;
  try {
    ...
    const totalMedicinePrice = receiptMedicines.reduce((sum, med) => sum + med.price, 0);
    const totalPrice = totalMedicinePrice + (doctorCharges || 0);
